...
void module_exit() {
    if (rtl8139_dev) {
        debugf("RTL8139: Module exiting, cleaning up...\n");
        rtl8139_cleanup();
        rtl8139_dev = NULL;
    }
}
...
void module_entry() {
    ...
    debugf("RTL8139 NIC found at bus %u, device %u, function %u\n",
    ...
    debugf("RTL8139: PCI command register set to 0x%x\n", cmd);

    // Get I/O base address from BAR0
    uint32_t io_base = rtl8139_dev->bar[0] & 0xFFFFFFFC; // Mask out lower bits
    if (!io_base) {
        debugf_warn("RTL8139 I/O BAR not set!\n");
        return;
    }

    debugf("RTL8139: I/O base address: 0x%x\n", io_base);
    ...
    debugf("RTL8139 MAC: %02x:%02x:%02x:%02x:%02x:%02x\n", mac[0], mac[1],
    ...
    debugf("RTL8139: Sending test ARP request...\n");
    rtl8139_send_arp_request(my_ip, target_ip);

    debugf("ARP request sent from %d.%d.%d.%d to %d.%d.%d.%d\n",
    ...
    debugf("RTL8139: Module initialization completed\n");
}