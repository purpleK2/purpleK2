...
const modinfo_t modinfo = {.name    = "ahci",
                           ...
                           .description =
                               "AHCI (SATA + ATAPI) Driver for PurpleK2",
                           .license  = "MIT",
...
static int ahci_sata_diskdev_read(struct disk_device *disk, uint8_t *buffer,
...
static int ahci_sata_diskdev_write(struct disk_device *disk,
                                   const uint8_t *buffer, uint64_t lba,
                                   uint32_t sector_count) {
    bool i = ahci_sata_write((HBA_PORT *)disk->data, lba, sector_count,
                             (void *)buffer);
...
static int ahci_atapi_diskdev_write(struct disk_device *disk,
                                    const uint8_t *buffer, uint64_t lba,
                                    uint32_t sector_count) {
    ...
    bool i = ahci_atapi_write((HBA_PORT *)disk->data, lba, sector_count,
                              (void *)buffer);
...
void module_entry() {
    ...
    for (int i = 0; i < 32; i++) {
        if (drivetypes[i] == DRV_SATA) {
            ...
            debugf_debug(
                "AHCI: Registered SATA disk at port %d as /dev/sd%c\n\tBlock "
                "size: %u bytes\n\tBlock count: %u blocks\n\tSize: %u MB\n",
                i, disk->id.letter, disk->block_size, disk->block_count,
                disk->block_size * disk->block_count / (1024 * 1024));
            ...
            while (current) {
                debugf_debug(
                    "  Partition: %s, Start LBA: %llu, Size: %llu sectors\n",
                    current->dev_path, current->start_lba,
            ...
            debugf_debug(
                "AHCI: Registered ATAPI disk at port %d as /dev/opt%c\n\tBlock "
                "size: %u bytes\n\tBlock count: %u blocks\n\tSize: %u MB\n",
                i, disk->id.letter, disk->block_size, disk->block_count,
                disk->block_size * disk->block_count / (1024 * 1024));
        }
